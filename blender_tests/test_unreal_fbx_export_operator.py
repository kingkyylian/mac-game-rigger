...
def run_test():
    ...
    mesh.name = "MGR_Unreal_Export_Mesh"

    export_path = Path(tempfile.gettempdir()) / "mac_game_rigger_unreal_export.fbx"
    qa_path = export_path.with_suffix(".qa.json")
    for path in (export_path, qa_path):
        if path.exists():
            path.unlink()

    bpy.context.scene.mgr_unreal_export_path = str(export_path)
    bpy.ops.object.select_all(action="DESELECT")
    mesh.select_set(True)
    armature.select_set(True)
    bpy.context.view_layer.objects.active = mesh

    assert bpy.ops.mgr.export_unreal_fbx() == {"FINISHED"}
    assert export_path.exists()
    assert export_path.stat().st_size > 0
    assert qa_path.exists()

    payload = json.loads(qa_path.read_text(encoding="utf-8"))
    assert payload["export_profile"] == "unreal_fbx"
    assert payload["bone_count"] == len(armature.data.bones)
    assert "Unreal FBX" in bpy.context.scene.mgr_export_message
...
print("MGR_UNREAL_FBX_EXPORT_OPERATOR_TEST_OK")
