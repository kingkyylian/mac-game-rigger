# Asset Evidence Progress Report

Schema status: **pass**
Production trial gate: **blocked**
Evidence file check: **enabled**
Evidence root: `.`

## Summary

- Slots: 25
- Real assets registered: 2
- Complete evidence entries: 2
- Score >= 3 count: 0
- Score >= 3 ratio: 0.0

## Production Trial Requirements

| Requirement | Status |
|---|---|
| `completeRealAssetsAtLeast10` | missing |
| `humanoidsAtLeast3` | missing |
| `quadrupedsAtLeast2` | missing |
| `lowPolyHumanoidIncluded` | pass |
| `thinLimbHumanoidIncluded` | missing |
| `wideShoulderOrBulkyIncluded` | missing |
| `tailCreatureIncluded` | missing |
| `propOrAccessoryIncluded` | missing |
| `score3PlusAtLeast70Percent` | missing |
| `unityImportPassesAtLeast3` | missing |
| `unrealPassOrExplicitBlocker` | pass |

## Missing Gate Items

- `completeRealAssetsAtLeast10`
- `humanoidsAtLeast3`
- `quadrupedsAtLeast2`
- `thinLimbHumanoidIncluded`
- `wideShoulderOrBulkyIncluded`
- `tailCreatureIncluded`
- `propOrAccessoryIncluded`
- `score3PlusAtLeast70Percent`
- `unityImportPassesAtLeast3`

## Category Counts

| Category | Complete Evidence Count |
|---|---:|
| humanoid | 2 |
| quadruped | 0 |
| tail creature | 0 |
| wing creature | 0 |
| prop | 0 |

## Slot Status

| Slot | Category | Real Asset | Evidence | Score | Unity | Unreal | Issues |
|---|---|---|---|---:|---|---|---|
| H-001 | humanoid | pass | pass | 2 | blocked | blocked |  |
| H-002 | humanoid | missing | missing |  |  |  |  |
| H-003 | humanoid | missing | missing |  |  |  |  |
| H-004 | humanoid | missing | missing |  |  |  |  |
| H-005 | humanoid | missing | missing |  |  |  |  |
| H-006 | humanoid | pass | pass | 1 | blocked | blocked |  |
| H-007 | humanoid | missing | missing |  |  |  |  |
| H-008 | humanoid | missing | missing |  |  |  |  |
| H-009 | humanoid | missing | missing |  |  |  |  |
| H-010 | humanoid | missing | missing |  |  |  |  |
| Q-001 | quadruped | missing | missing |  |  |  |  |
| Q-002 | quadruped | missing | missing |  |  |  |  |
| Q-003 | quadruped | missing | missing |  |  |  |  |
| Q-004 | quadruped | missing | missing |  |  |  |  |
| Q-005 | quadruped | missing | missing |  |  |  |  |
| C-001 | tail creature | missing | missing |  |  |  |  |
| C-002 | wing creature | missing | missing |  |  |  |  |
| C-003 | tail creature | missing | missing |  |  |  |  |
| C-004 | wing creature | missing | missing |  |  |  |  |
| C-005 | tail creature | missing | missing |  |  |  |  |
| P-001 | prop | missing | missing |  |  |  |  |
| P-002 | prop | missing | missing |  |  |  |  |
| P-003 | prop | missing | missing |  |  |  |  |
| P-004 | prop | missing | missing |  |  |  |  |
| P-005 | prop | missing | missing |  |  |  |  |
