...
| Strict separate-mesh humanoid gate | blocked | `docs/asset-evidence-progress.md` shows `realSeparateMeshHumanoidEvidence` blocked for H-003, H-004, H-005, H-006, H-009, and H-010. Current complete score >= 3 real humanoid workflow summaries all report `meshCount` 1, so no real separate-mesh hair/accessory/clothing humanoid evidence exists yet. |
...
- Strict separate-mesh humanoid evidence is missing: all complete score >= 3 real humanoid workflow summaries currently report `meshCount` 1, so hair/accessory/clothing split-mesh behavior is still unproven on real assets.
