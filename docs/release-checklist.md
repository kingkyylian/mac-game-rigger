...
| Real asset evidence gate | pass | `scripts/validate_asset_evidence.py --manifest samples/manifest.json --evidence-root . --check-evidence-files --require-production-trial --quiet` passes with 13 complete real asset evidence entries and 12 score >= 3 entries. `docs/asset-evidence-progress.md` keeps stricter game-ready gates separate so the production-trial pass does not hide product gaps. |
| CI smoke gate | pass | `.github/workflows/ci.yml` runs `scripts/run_full_alpha_smoke.sh --skip-blender` and uploads `dist/MacGameRigger-0.1.0.zip`; verify the GitHub Actions run for each pushed SHA because run IDs are intentionally not pinned in this checklist. |
...
| Strict humanoid Animator gate | pass | `docs/asset-evidence-progress.md` shows `configuredAnimatorSmokeForHumanoidScore3` pass. `scripts/plan_unity_animator_smoke_migration.py --manifest samples/manifest.json --evidence-root . --json` reports no gaps, and strict validation with `--require-configured-animator-smoke` passes. H-002, H-003, H-004, H-005, H-006, H-009, and H-010 all record passing configured Animator smoke. |
| Humanoid Avatar smoke gate | pass | `tools/unity_import_check/Assets/Editor/MacGameRiggerFbxImportCheck.cs` emits `humanoidAvatarSmoke` using Unity `AvatarBuilder.BuildHumanAvatar`. `docs/asset-evidence-progress.md` shows `humanoidAvatarSmokeForHumanoidScore3` pass; H-002, H-003, H-004, H-005, H-006, H-009, and H-010 all record valid human Avatars with 17/17 required human bones mapped. |
| Strict separate-mesh humanoid gate | pass | `docs/asset-evidence-progress.md` shows `realSeparateMeshHumanoidEvidence` pass. `scripts/verify_split_mesh_humanoid_evidence.py --manifest samples/manifest.json --evidence-root . --json` passes with H-002: source mesh count 8, rig mesh count 8, deformation score 3. |
| Split-mesh humanoid intake plan | pass | `scripts/plan_split_mesh_humanoid_intake.py --manifest samples/manifest.json --json` reports H-007 and H-008 as the remaining open humanoid slots and recommends H-007. When an asset path is provided, it emits source import smoke, Blender workflow, manifest registration, and report regeneration commands. |
...
| Split-mesh candidate preflight | pass | `scripts/preflight_split_mesh_candidate.py --candidate kaykit-adventurers --source-smoke <asset-import-smoke.json> --json` validates source import smoke before workflow intake. It blocks single-mesh source imports; for example H-003 source smoke reports mesh count 1 and is rejected. |
| Split-mesh intake runner | pass | `scripts/run_split_mesh_humanoid_intake.py --candidate kaykit-adventurers --asset <asset> --dry-run --json` builds the guarded source smoke -> candidate preflight -> Blender workflow -> review packet sequence. In execute mode it checks `workflow-summary.json` rig mesh count > 1, creates `notes.md` with manual review placeholders, and stops at `needs_registration_review` instead of auto-registering evidence without manual deformation score and visual review. |
| Split-mesh humanoid verifier | pass | `scripts/verify_split_mesh_humanoid_evidence.py --manifest samples/manifest.json --evidence-root . --json` passes with H-002 as the current real split-mesh humanoid proof. |
...
scripts/validate_asset_evidence.py --manifest samples/manifest.json --evidence-root . --check-evidence-files --require-production-trial --require-configured-animator-smoke --quiet
...
scripts/run_split_mesh_humanoid_intake.py --manifest samples/manifest.json --candidate kaykit-adventurers --asset local_assets/H-002/kaykit-adventurer.glb --dry-run --json
scripts/create_split_mesh_review_packet.py --slot H-002 --source-smoke evidence/H-002/asset-import-smoke.json --workflow-summary evidence/H-002/workflow-summary.json --output evidence/H-002/notes.md --json
scripts/preflight_split_mesh_candidate.py --candidate kaykit-adventurers --source-smoke evidence/H-003/asset-import-smoke.json --json
...
- `scripts/preflight_split_mesh_candidate.py --candidate kaykit-adventurers --source-smoke evidence/H-003/asset-import-smoke.json --json` is expected to return exit code `1` because H-003 source import smoke reports mesh count `1`; this proves the split-mesh preflight rejects single-mesh humanoid sources before intake.

## Known issues

- Unity configured Animator evidence is now recorded for all score >= 3 Unity-pass humanoids, but Unity batchmode still must run outside the sandbox because Unity Package Manager cannot open its local socket there. If Unity Licensing Client stalls again, kill the stale licensing process or open Unity Hub/Editor once before rerunning the recorder.
- Humanoid Avatar smoke is recorded for all current score >= 3 Unity-pass humanoids and the strict report gate passes. This proves Avatar construction and basic retarget readiness, but does not yet prove playback of third-party humanoid animation clips.
...
- GitHub Actions CI is configured and the latest checked run passes; keep local smoke verification in the release path because Blender and Unity engine checks still depend on local macOS installations.
- Strict configured Animator smoke and Humanoid Avatar smoke are closed for current score >= 3 Unity-pass humanoids. Third-party retarget clip playback is still a future gate.
- Strict separate-mesh humanoid evidence is closed by H-002, but broader hair/accessory/clothing behavior still needs more than one real split-mesh humanoid before a strong production-quality claim.
- Split-mesh humanoid intake is planned by script and KayKit Adventurers is registered as a primary CC0 candidate; H-002 now proves the guarded intake path on one real GLB source.
...
- Production trial evidence and the two stricter game-ready gates now pass, but textured/material QA, manual cleanup scoring, Unreal validation, and broader split-mesh coverage remain product gaps.
- Full Blender rig workflow timing now has one real H-006 baseline, a 10-slot real asset family benchmark, synthetic 10k / 50k / 100k scalability baselines, and synthetic template-family timing; additional real separate-mesh hair/accessory humanoids and Blender 4.2 timing still need evidence.
