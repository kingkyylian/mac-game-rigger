...
| Smoke benchmark | partial | `docs/alpha-smoke-results.md` has 5 generated proxy rows, but real Unity/Unreal import is blocked. |
...
- Unity/Unreal engine import validation is blocked: Unity Hub is installed, but no Unity Editor binary was found; `UnrealEditor` is not available on `PATH`.
